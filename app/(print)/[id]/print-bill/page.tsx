...
function groupByDate(items: any[]) {
  return items.reduce((acc, item) => {
    const date = format(new Date(item.created_at || item.prescription_date || item.test_date || item.surgery_date), "yyyy-MM-dd");
    if (!acc[date]) {
      acc[date] = [];
    }
    acc[date].push(item);
                ...
                <table className="w-full border-collapse border border-gray-300">
                  ...
                  <tbody>
                    {groupedSurgeries[date].map((surgery: any) => {
                      totalBill += Number(surgery.cost);
                      return (
                        <tr key={surgery.surgery_id}>
                          <td className="border border-gray-300 px-4 py-2">{surgery.surgery_name}</td>
                          <td className="border border-gray-300 px-4 py-2">${Number(surgery.cost).toFixed(2)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
