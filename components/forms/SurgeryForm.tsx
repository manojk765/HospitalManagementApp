...
export default function SurgeryForm({ initialData, onSubmit }: SurgeryFormProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push('/list/services/surgery')
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Hidden input for surgery_name when in edit mode */}
        {initialData?.surgery_name && (
          <input
            type="hidden"
            name="surgery_name"
            value={initialData.surgery_name}
          />
        )}
        
        <div>
          <label htmlFor="surgery_name_display" className="block text-sm font-medium text-gray-700 mb-2">
            Surgery Name
          </label>
          <input
            type="text"
            id="surgery_name_display"
            required={!initialData}
            defaultValue={initialData?.surgery_name || ''}
            disabled={!!initialData?.surgery_name}
